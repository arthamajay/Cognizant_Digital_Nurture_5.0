/* ============================================================
   Local Community Event Portal — Main Application
   ============================================================ */
...
const allEvents = [
  { id:1, title:"Summer Music Festival",  category:"music",    date:"2026-06-15", location:"Central Park Amphitheater", seats:120, fee:15,
  ...
  { id:2, title:"Baking Workshop",         category:"workshop", date:"2026-06-20", location:"Community Center Hall B",   seats:30,  fee:10,
  ...
  { id:3, title:"City Marathon 5K",        category:"sports",   date:"2026-07-04", location:"Riverside Trail",           seats:200, fee:5,
  ...
  { id:4, title:"Cultural Heritage Fair",  category:"culture",  date:"2026-07-12", location:"Town Square",               seats:500, fee:0,
  ...
  { id:5, title:"Yoga in the Park",        category:"health",   date:"2026-06-28", location:"Greenfield Park",           seats:50,  fee:8,
  ...
  { id:6, title:"Jazz Night",              category:"music",    date:"2026-08-01", location:"Riverside Pavilion",        seats:80,  fee:12,
...
function getValidEvents(events) {
  ...
  return events.filter(ev => ev.date >= today && ev.seats > 0);
}
...
function EventModel(id, title, category, date, location, seats, fee, description, image) {
  ...
  this.fee = fee; this.description = description; this.image = image;
}
...
function addSampleEvent() {
  allEvents.push(new EventModel(
    allEvents.length + 1, "Photography Walk", "culture", "2026-09-10",
    "Old Town District", 40, 0, "Guided photography tour through historic streets.",
...
function renderEvents(events) {
  ...
  if (events.length === 0) {
    container.innerHTML = `<p class="text-center text-muted" style="width:100%;padding:40px 0;">
      No events found. Try a different filter or search term.</p>`;
    return;
  }
  ...
  events.forEach(ev => {
    ...
    card.innerHTML = `
      <img class="event-card-img" src="${ev.image}" alt="${ev.title}" title="Double-click to enlarge"
           ondblclick="openLightbox('${ev.image}', '${ev.title}')">
      <div class="event-card-body">
        <span class="badge badge-${ev.category}">${ev.category}</span>
        <h3 class="event-card-title" style="margin-top:8px;">${ev.title}</h3>
        <p class="event-card-meta">📅 ${formatDate(ev.date)}<br>📍 ${ev.location}</p>
        <p style="font-size:0.88rem;color:#4a5568;">${ev.description}</p>
        <div class="event-card-footer">
          <span class="${seatClass}">🪑 ${ev.seats} seats</span>
          <span style="font-weight:700;color:var(--primary);">${feeText}</span>
          <button class="btn btn-primary btn-sm" onclick="registerUser(${ev.id})">Register</button>
        </div>
      </div>`;
...
function fetchEventsPromise() {
  ...
  fetch("events.json")
    ...
    .then(data => {
      ...
      if (container) container.innerHTML = `<p class="text-center" style="color:var(--success);font-weight:600;">
        ✅ Loaded ${data.length} events from server.</p>`;
...
async function fetchEventsAsync() {
  ...
  try {
    ...
    if (spinner) spinner.classList.remove("show");
    if (container) container.innerHTML = `<p class="text-center" style="color:var(--success);font-weight:600;">
      ✅ Loaded ${data.length} events via async/await.</p>`;
...
function createEventCard(title, category = "general", fee = 0) {
  return { title, category, fee };
}

function displayEventDetails(event) {
  const { title, date, location, seats, fee } = event;
...
function handleRegistrationForm(e) {
  ...
  const date  = form.elements["reg-date"].value;
  const type  = form.elements["reg-type"].value;
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    showFieldError("email-error", "Enter a valid email address."); valid = false;
  } else { hideFieldError("email-error"); }

  if (!date) { showFieldError("date-error", "Please select a date."); valid = false; }
  else { hideFieldError("date-error"); }

  if (!valid) return;

  const output = document.getElementById("reg-output");
  if (output) {
    output.textContent = `✅ Thank you, ${name}! You're registered for "${type}" on ${formatDate(date)}.`;
    output.classList.add("show");
  }

  localStorage.setItem("preferredEventType", type);
  updatePrefStatus();
  postRegistration({ name, email, date, type, message: msg });
  form.reset();
}
...
function postRegistration(data) {
  ...
  setTimeout(() => {
    fetch("https://jsonplaceholder.typicode.com/posts", {
      ...
      .then(res => res.json())
      .then(result => { console.log("Server response:", result); showToast("Registration submitted!"); })
      .catch(err => { console.error("POST error:", err); showToast("Saved locally.", true); });
  }, 800);
}
...
function showEventFee(selectEl) {
  const fees = { music:15, workshop:10, sports:5, culture:0, health:8 };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (regForm) {
    regForm.addEventListener("input",  () => { formDirty = true; });
    regForm.addEventListener("submit", () => { formDirty = false; });
  }
});
...
function loadPreferences() {
  ...
  if (fbType) { fbType.value = saved; showEventFee(fbType); }
  updatePrefStatus();
...
function findNearbyEvents() {
  ...
  if (!navigator.geolocation) {
    if (resultEl) { resultEl.innerHTML = "❌ Geolocation is not supported by your browser."; resultEl.classList.add("show"); }
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      if (resultEl) {
        ...
        resultEl.innerHTML = `
          <strong>📍 Your Location Found!</strong><br>
          Latitude: <code>${latitude.toFixed(5)}</code><br>
          Longitude: <code>${longitude.toFixed(5)}</code><br>
          Accuracy: <code>±${Math.round(accuracy)} meters</code><br><br>
          <strong>Nearest Events:</strong>
          <ul style="margin-top:8px;padding-left:20px;">
            <li>🎵 Summer Music Festival — Central Park (2.1 km)</li>
            <li>🏃 City Marathon 5K — Riverside Trail (3.4 km)</li>
            <li>🧘 Yoga in the Park — Greenfield Park (1.8 km)</li>
          </ul>`;
      }
